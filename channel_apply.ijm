...
for (p = 0; p < lengthOf(files); p++) {
	...
	settings_String = File.openAsString(settings_file_path);
		
	// get variables from settings file
	File_Name = getSetting(settings_String, 1);
	XY_registration = getSetting(settings_String, 2);
	crop_output = getSetting(settings_String, 7);
	z_registration = getSetting(settings_String, 8);
	reslice_mode = getSetting(settings_String, 10);
	extend_stack_to_fit = getSetting(settings_String, 14);
	ram_conservative_mode = getSetting(settings_String, 15);
	DriftTable_path_XY = getSetting(settings_String, 16);
	DriftTable_path_Z = getSetting(settings_String, 17);
...
function getSetting(settings_string, index){
	settings_array = split(settings_String, "\n");
	settings_row = split(settings_array[index], ",");
	return settings_row[2];
}
